    ...
    for split in range(3):
        ...
        for e in range(options.n_epochs):
            ...
            for i, sample in enumerate(trainloader):
                ...
                d_matrix = (1 - gamma) * dist_matrix(s_word, all_class_embeddings) + gamma * dist_matrix(s_attr,
                                                                                                         all_class_predicates)

                l2_dist_loss = (d_matrix * same_class.float()).mean()
                l2_loss = (1 - gamma) * (s_word * s_word).sum(dim=-1).mean() + gamma * (s_attr * s_attr).sum(
                    dim=-1).mean()
