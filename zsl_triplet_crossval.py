...
def parse_args():
    ...
    parser.add_argument('-only_words', action='store_const', const=True, default=False, help='Only use class word embeddings')
    parser.add_argument('-use_resnet', action='store_const', const=True, default=False, help='use ResNet101 image embeddings')

    parser.add_argument('--batch_size', type=int, default=128)
    parser.add_argument('--n_epochs', type=int, default=100)
    parser.add_argument('--optimizer', type=str, default='sgd', help='\'sgd\'(default) or \'adam\'')

    parser.add_argument('--learning_rates', nargs='+', type=float, default=[5e-3, 1e-3, 5e-4], help='learning_rates for each split : (default) 5e-3 1e-3 5e-4')
...
def main():
    ...
    # Load Data
    if options.leonhard:
        train_path = 'ZSL_Data/AwA2/train_set'
        validation_path = 'ZSL_Data/AwA2/val_set'

    else:
        train_path = 'Data/AwA2/train_set'
        validation_path = 'Data/AwA2/val_set'

    train_set_1 = ZSLDataset(train_path+'_1',
                             use_predicates=not options.only_words,
        ...
        if not options.only_words:
            ...
            if options.optimizer == 'adam':
                optimizer = torch.optim.Adam(list(v_to_s.parameters()) + list(s_to_v.parameters()),
                                             lr=options.learning_rates[split],
                                             betas=(0.9, 0.999),
                                             weight_decay=3e-3)
            else:
            ...
            split_scores.append(np.mean(validation_accuracy))
            print('split {0} done\n'.format(split))

        else:
            v_to_s = ContinuousMap(dim_source=dim_visual,
                                   dim_dest=dim_semantic,
                                   width=512).cuda()
            s_to_v = ContinuousMap(dim_source=dim_semantic,
                                   dim_dest=dim_visual,
                                   width=512).cuda()
    ...
    print('Split scores:\n')
    for i in range(3):
        print('Split {0} (lr {1}) Mean Accuracy: {2}'.format(i+1, options.learning_rates[i], split_scores[i]))
